...
func (c *Congress) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	if !isEpoch && validatorsBytes != 0 {
		log.Info("verifyHeader", "isEpoch", isEpoch, "number", number, "c.config.Epoch", c.config.Epoch, "validatorsBytes", hexutil.Encode(header.Extra))
...
func (c *Congress) initializeSystemContracts(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	for _, contract := range contracts {
		...
		nonce := state.GetNonce(header.Coinbase)
		msg := newMessage(header.Coinbase, &contract.addr, nonce, new(big.Int), math.MaxUint64, new(big.Int), data, true)
...
func (c *Congress) getTopValidators(chain consensus.ChainHeaderReader, header *types.Header) ([]common.Address, error) {
	...
	msg := newMessage(header.Coinbase, &validatorsContractAddr, 0, new(big.Int), math.MaxUint64, new(big.Int), data, false)
...
func (c *Congress) updateValidators(vals []common.Address, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	// call contract
	nonce := state.GetNonce(header.Coinbase)
	msg := newMessage(header.Coinbase, &validatorsContractAddr, nonce, new(big.Int), math.MaxUint64, new(big.Int), data, true)
	if _, err := executeMsg(msg, state, header, newChainContext(chain, c), c.chainConfig); err != nil {
...
func (c *Congress) punishValidator(val common.Address, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	// call contract
	nonce := state.GetNonce(header.Coinbase)
	msg := newMessage(header.Coinbase, &punishContractAddr, nonce, new(big.Int), math.MaxUint64, new(big.Int), data, true)
	if _, err := executeMsg(msg, state, header, newChainContext(chain, c), c.chainConfig); err != nil {
...
func newMessage(from common.Address, to *common.Address, nonce uint64, amount *big.Int, gasLimit uint64, gasPrice *big.Int, data []byte, checkNonce bool) core.Message {
	return core.Message{
		To:                to,
		From:              from,
		Nonce:             nonce,
